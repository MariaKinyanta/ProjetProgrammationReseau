...
class BankClientApp:
    def __init__(self, root):
        ...
        self.client_name = ""
        self.user_pin = None  # Pour confirmation d'opérations
        ...
        self.style.configure("Header.TLabel", background=navy, foreground=doré, font=("Segoe UI", 16, "bold"))
        self.style.configure("TButton", font=("Segoe UI", 12, "bold"), padding=6,
                             background=doré, foreground=navy, borderwidth=0)
        self.style.map("TButton", background=[("active", "#b7950b"), ("!active", doré)])
        self.style.configure("Treeview", background="white", foreground=navy, fieldbackground="white", font=("Segoe UI", 11))
        self.style.configure("Treeview.Heading", background=doré, foreground=navy, font=("Segoe UI", 12, "bold"))
        # Style spécifique pour les cadres de connexion/inscription
    ...
    def create_registration_screen(self):
        ...
        # Le numéro de compte sera généré par le serveur
        ...
        # Bouton de retour vers l'écran de connexion
    ...
    def register_user(self, pin, nom, prenom, adresse, cp, ville, tel_fixe, tel_portable, type_compte):
        ...
        print("Commande envoyée au serveur:", command)  # Debug
        response = socket_handler.send_request(command, SERVER_HOST, SERVER_PORT, CAFILE)
        print("Réponse du serveur:", response)  # Debug
    ...
    def create_main_screen(self):
        self.clear_screen()
        # Création d'un Notebook pour organiser deux onglets : Accueil et Historique
    ...
    def logout(self):
        # Envoi d'une commande DECONNECTER si nécessaire sur le serveur
